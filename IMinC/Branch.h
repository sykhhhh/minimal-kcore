...
public:
	...
	void KCore()
	{
		...
		for (int u = 0; u < g.n; u++)
		{
			...
			vector<int> C;
			vis[u]=1;
			for(int v:g.vertices[u].neighbors)
			{
				if(v<u) continue;
				if(vis[v]==-1)
				{
				...
				for(int x:g.vertices[v].neighbors)
				{
					if(x<u) continue;
					if(vis[x]==-1)
					{
			...
			do // reduce C
			{
				...
				subg.two_hop_reduction(k, q, idx);
				subg.strong_reduce(k,q);
