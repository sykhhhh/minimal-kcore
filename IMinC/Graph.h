...
public:
	...
	void strong_reduce(int k, int q)
	{
		int m_before = m;
		do
		{
			vertex_reduction(k, q);
			m_before = m;
			edge_reduction(k, q);
		} while (m_before > m);
	}
	void edge_reduction(int k, int q)
	{
		vector<bool> vis(n);
		...
		for (auto &vertex : vertices)
		{
			...
			for (int i = 0; i < vertex.neighbors.size(); i++)
			{
				...
				{
					if (same_neighbor>=lb)
						neighbors[new_size++] = v;
					else mp[make_pair(v,u)]=1;
				}
			}
			m -= neighbors.size() - new_size;
			neighbors.resize(new_size);
			vertex.d = new_size;
			for (int v : vertex.neighbors)
				vis[v] = 0;
		}
	}
	void vertex_reduction(int k, int q)
	{
